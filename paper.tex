...
% Commented out inclusion of algorithm2e environment because these are now separate figures in figs_plos subdir - IH, 7/17/09
...
%  1: inside.eps
%  2: insidetransemit.eps
%  3: cyk.eps
%  4: cyktransemit.eps
%  5: cyktraceback.eps
%  6: outside.eps
%  7: outsidetransemit.eps
%  8: stree-with-structure.eps
%  9: stree-mutations-with-structures.eps
% 10: perfect.eps
% 11: ama.eps
...
The reconstruction and synthesis of ancestral RNAs is a feasible goal for paleogenetics.
This will require new bioinformatics methods, including a
robust statistical framework for reconstructing histories of
substitutions, indels and structural changes.
...
We implemented the above algorithms for a simple model of pairwise RNA
structural evolution; in particular the algorithms for maximum
likelihood (ML) alignment of three known RNA structures an a known
phylogeny and inference of the common ancestral structure.  We
compared this ML algorithm to a variety of related, but simpler,
techniques, including ML alignment algorithms for simpler models that
omitted various aspects of the full model and also a
posterior-decoding alignment algorithm for one of the simpler models.

In our tests, incorporation of basepair structure was the most
important factor for accurate alignment inference; appropriate use
of posterior-decoding was next; and fine
details of the model were least important.  Posterior-decoding
heuristics can be substantially faster than exact phylogenetic
inference, so this motivates the use of sum-over-pairs heuristics where possible
(and approximate sum-over-pairs).  For more exact
probabilistic inference, we discuss the use of transducer composition
for ML (or MCMC) inference on phylogenies, including possible ways to
make the core operations tractable.
...
\subsubsection*{Terminology and normalization}
...
$X$, $Y$, and $Z$ the (observed) extant sequences.
% (as in Figures \ref{fig:parent} and \ref{fig:threeway}).
...
The Inside, Outside, and CYK+traceback algorithms for a three-taxon star phylogeny
can be constrained using the ``fold envelope'' concept, which will now be described.
...
An inside$\to$outside ordering is used for the iteration 
...
The Outside algorithm uses the exact reverse of the inside$\to$outside ordering described above; we call this the outside$\to$inside ordering.
...
Using the fold envelope formalism, the main iteration over cells in the Inside and CYK matrices can be expressed as three nested loops:
one for each sequence, traversing the fold envelope subsequences in inside$\to$outside order.
Conversely, the main iteration of the Outside algorithm consists of three nested outside$\to$inside loops.
...
The grammars are presented in \indiegram\ format in \supptext{4}.
...
model construction.  \dataset{1} gives the state space of the ensemble
model constructed by the search algorithm and \dataset{2} the reduced
...
Theory predicts that Maximum Likelihood inference, using the correct model and parameters, should be asymptotically optimal (if one only counts perfect guesses).
Inspecting \figref{perfect}, we find this to be almost the case; the exception is when the outgroup is very distant and the bins may be undersampled
(the departure from prediction that we observe for low-identity alignments is not statistically significant:
when the optimal success rate drops below $\sim 5/125$, then 125 trials are probably insufficient to compare two near-optimal methods).
...
In \figref{ama}, we abandon the all-or-nothing metric of counting only perfect alignments,
...
\figref{ama} reveals that \stemlocama\ (which attempts to find the alignment with the maximum expected AMA)
...
% The following one-liner was used to convert all algorithms into standalone tex files for use as figures
% cat paper.tex | perl -e '@x=<>;$x=join("",@x);$x=~s/\r\n/\n/g;while($x=~/.begin.algorithm.(.*?).caption.*?.alglabel.([^\}]+)/sg){($a,$f)=($1,$2);open F,">figs_plos/$f.tex";print F "\\documentclass[10pt]{article}\n\\input{algdefs.tex}\n\\begin{document}\n\\begin{algorithm}$a\n\\end{algorithm}\n\\end{document}\n";close F}'
...
{\bf    The constrained Inside algorithm for three sequences $X, Y, Z$. }
...
The subroutine $\mathrm{calcTransEmitProb}()$ is given in \algref{insidetransemit}.
  }
\end{figure}
...
{\bf    Subroutine $\mathrm{calcTransEmitProb}()$ for the Inside algorithm. }
    $\bvec{a}$ and $\bvec{b}$ are ensemble states; $\bvec{l}$ and $\bvec{r}$ are left and right terminal emissions.
The Inside algorithm itself is given in \algref{inside}.
  }
\end{figure}
...
{\bf    The constrained CYK algorithm for three sequences $X, Y, Z$. }
    Ensemble states $\bvec{a}$ in the iteration over states are sorted in Inside fill order
    with $\Semit$ states first, then $\Snull$ states in reverse topological order.
The subroutine $\mathrm{calcTransEmitProb}()$ is given in \algref{cyktransemit}.
The algorithm to find the Maximum Likelihood parse tree by traceback through the CYK matrix is given in \algref{cyktraceback}.
  }
\end{figure}
...
{\bf    Subroutine $\mathrm{calcTransEmitProb}()$ for the CYK algorithm. }
    $\bvec{a}$ and $\bvec{b}$ are ensemble states; $\bvec{l}$ and $\bvec{r}$ are left and right terminal emissions.
The CYK algorithm itself is given in \algref{cyk}.
  }
\end{figure}
...
{\bf     The constrained CYK traceback algorithm for three sequences $X, Y, Z$.}
For compactness we show only the ``if'' case for left-bifurcations ($\bvec{a} \to \bvec{c} \bvec{b}$);
the case for right-bifurcations ($\bvec{a} \to \bvec{b} \bvec{d}$) is identical,
with $(\bvec{c},\bvec{b})$ respectively replaced by $(\bvec{b},\bvec{d})$.
The CYK algorithm itself is given in \algref{cyk}.
  }
\end{figure}
...
{\bf    The constrained Outside algorithm for three sequences $X, Y, Z$. }
...
The subroutine $\mathrm{calcTransEmitProb}()$ is given in \algref{outsidetransemit}.
  }
\end{figure}
...
{\bf    Subroutine $\mathrm{calcTransEmitProb}()$ for the Outside algorithm. }
    $\bvec{a}$ and $\bvec{b}$ are ensemble states; $\bvec{l}$ and $\bvec{r}$ are left and right terminal emissions.
The Outside algorithm itself is given in \algref{outside}.
  }
\end{figure}
...
\begin{table}[!ht]
  ...
  \begin{tabular}{|l|r||l|r|}
    ...
    $\phantom{L} \rightarrow B$ & $\kappa_l \cdot \pi_l(S)$ & $\phantom{S} \rightarrow B_e$ & $1-\kappa_s$ \\ \hline
    $\phantom{L} \rightarrow \Send$ & $1-\kappa_l$ & & \\ \hline
    $I_L \rightarrow u \, \, I_L$ & $\kappa_l \cdot \pi_l(u)$ & $I_S \rightarrow u \,\, I_S \,\, v$ & $\kappa_s \cdot \pi_s(uv)$\\ \hline
    $\phantom{I_L} \rightarrow B$ & $\kappa_l \cdot \pi_l(S)$ & $\phantom{I_S} \rightarrow B_e$ & $1-\kappa_s$\\ \hline
    $\phantom{I_L} \rightarrow \Send$ & $1-\kappa_l$ & & \\ \hline \hline
...
\begin{table}[!ht]
  ...
  \begin{tabular}{|l|r||l|r|} \hline
    ...
    $\phantom{L} \rightarrow B_i$ & $\beta_l(t) \cdot \pi_l(S)$ & $\phantom{S} \rightarrow W_S$ & $1-\beta_s(t)$ \\ \hline
    $\phantom{L} \rightarrow W_L$ & $1-\beta_l(t)$ & & \\ \hline
    $I_L \rightarrow w \,\, I_L$ & $\beta_l(t) \cdot \pi_l(w)$ & $I_S \rightarrow w \,\, I_S \,\, x$ & $\beta_s(t) \cdot \pi_s(wx)$ \\ \hline
    $\phantom{I_L} \rightarrow B_i$ & $\beta_l(t) \cdot \pi_l(S)$ & $\phantom{I_S} \rightarrow W_S$ & $1-\beta_s(t)$ \\ \hline
    $\phantom{I_L} \rightarrow W_L$ & $1-\beta_l(t)$ & & \\ \hline
    $M_L \rightarrow w \,\, I_L$ & $\beta_l(t) \cdot \pi_l(w)$ & $M_S \rightarrow w \,\, I_S \,\, x$ & $\beta_s(t) \cdot \pi_s(wx)$ \\ \hline
    $\phantom{M_L} \rightarrow B_i$ & $\beta_l(t) \cdot \pi_l(S)$ & $\phantom{M_S} \rightarrow W_S$ & $1-\beta_s(t)$ \\ \hline
    $\phantom{M_L} \rightarrow W_L$ & $1-\beta_l(t)$ & & \\ \hline
    $D_L \rightarrow w \,\, I_L$ & $\gamma_l(t) \cdot \pi_l(w)$ & $D_S \rightarrow w \,\, I_S \,\, x$ & $\gamma_s(t) \cdot \pi_s(wx)$ \\ \hline
    $\phantom{D_L} \rightarrow B_i$ & $\gamma_l(t) \cdot \pi_l(S)$ & $\phantom{D_S} \rightarrow W_S$ & $1-\gamma_s(t)$ \\ \hline
    $\phantom{D_L} \rightarrow W_L$ & $1-\gamma_l(t)$ & & \\ \hline
    $W_L \rightarrow w \,\, M_L$ & $\alpha_l(t) \cdot M_l(u \rightarrow w)$ & $W_S \rightarrow w \,\, M_S \,\, x$ & $\alpha_s(t) \cdot M_s(uv \rightarrow wx)$ \\ \hline
    $\phantom{W_L} \rightarrow D_L$ & $1-\alpha_l(t)$ & $\phantom{W_S} \rightarrow D_S$ & $1-\alpha_s(t)$ \\ \hline
    $\phantom{W_L} \rightarrow B$ & $\alpha_l(t)$ & $\phantom{W_S} \rightarrow B_e$ & $1$ \\ \hline
    $\phantom{W_L} \rightarrow B_p$ & $1-\alpha_l(t)$ & & \\ \hline
    $\phantom{W_L} \rightarrow \Send$ & $1$ & & \\ \hline \hline
