...
near-impossible for very long outgroup branch lengths, the same need not
true for structural reconstruction accuracy.  As the outgroup branch
length increases, its structure becomes increasingly decoupled from
that of its ancestor, allowing the algorithm to ``ignore'' the
information which it contains.
Similarly, while we can perfectly reconstruct the ancestral
structure given an outgroup branch length of 0 (the trivial case), we
cannot perfectly reconstruct the alignment.  
...
\begin{figure}[!ht]
  ...
  \caption{
    ...
    Parameters used were $\lambda_1 = 0.025$ and $\mu_1 = 0.03$ for
    loop sequence and $\lambda_2 = 0.007$ and $\mu_2 = 0.01$ for stem
    sequence; the probability of a stem insertion was $0.1$.  These
    yielded a mean loop length of 5 and a mean stem length of 2.33,
    ...
    10]$ and stems of length $\in [1, 20]$; to reduce the complexity of
