...
The model construction algorithm described above creates a phylogenetic SCFG
which generates a joint distribution over both ancestral and extant
sequences. A parse tree for this SCFG represents a structural and evolutionary
explanation of the extant sequences, including a complete ancestral reconstruction.
Therefore, given a set of extant sequences, many of the questions of interest to us
can be reduced to searches over, or summarizations of, the set of possible parse trees.

Well-known algorithms exist for searching or summing over SCFG parse trees.
...
and the Inside-Outside algorithm yields posterior probabilities for individual parse tree nodes.

All of these algorithms are, however, complicated for phylogenetic SCFGs by
the possibility of fragments of ancestral sequence that were universally deleted and therefore
are unobserved in any of the extant sequences.
These unobserved fragments can be unbounded in length.
In the parse tree, these fragments correspond to cyclic detours through $\Snull$ states,
along with bifurcations where one or both sides terminate without observable emissions
(e.g. $B \to X\ Y$ followed by $X \to \epsilon$).
In the CYK, Inside and Outside recursions, this causes cyclic dependencies which cannot be resolved.

Therefore, before presenting equivalents of the CYK, Inside and Outside algorithms,
we first give a method to eliminate ``null cycles'' from
the ensemble model, by transforming any SCFG to an equivalent acyclic SCFG.
Some such step is required, in order to deal with cyclic dependencies, but alternative approaches to ours are clearly possible.
A CYK-only approach appears in the computational linguistics literature \cite{GecsegSteinby97}.
We have also developed an independent heuristic that iteratively re-fills entries of the DP matrix
until the approximation is sufficiently good.
...
Empty paths and empty bifurcations can be combined to give finite-probability sequences of rules
...
interest.  We can use this knowledge as a constraint to reduce the accessible volume of the DP matrix \cite{Holmes2005}.
