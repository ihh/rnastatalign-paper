...
compatible with a curated multiple sequence alignment.  Such constraints are
...
A potentially-unbounded number of residues in the ancestor can be
...
We therefore developed a method to remove null cycles from
the ensemble model, described below, in order to use the standard inference algorithms
...
In all such models, null cycles represent histories that are valid
...
using a MCMC kernel similar to that of \cite{JensenHein2002}.
...
Using our uninformed search algorithm, we determined that there are 287 accessible states
...
suggested in ``The composition algorithm.''

% to do: dump output of tripletscfg.pl --tkfst into supplement
Text S1 contains the complete model specification, including states and transitions,
