...
(when the perfect alignment rate drops below $\sim 5/125$, 125 trials may be too few).
...
Taken together, these results show that the most important factor distinguishing the various models
is the incorporation of some form of basepair structure:
structure-less \handel\  (regardless of linear {\em vs} affine gap penalty) performs much worse than the structure-aware SCFG methods.
...
The second most important, amongst the factors we have considered in this experiment, is use of the correct posterior-decoding algorithm for the objective function at hand
(c.f. \stemloc\ {\em vs} \stemlocama).
An obvious corollorary is the importance of choosing an appropriate objective function in the first place
...
\begin{figure}[!ht]
  ...
  \caption{
    ...
    thus, for example, the bin named ``$0 to 0.4$'' includes $t \in \{ 0, 0.1, 0.2, 0.3, 0.4 \}$.
