...
and to investigate the performance of our reconstruction method (when simulating directly from the model)
compared to simpler reconstruction methods that ignore either structure or phylogeny.
...
demonstrate that while the difficulty of alignment inference increases
in proportion to the outgroup branch length, becoming very challenging
for long outgroup branch lengths, the same need not be true for
structural reconstruction accuracy.  As the branch length of the
outgroup increases, the outgroup's structure becomes increasingly
decoupled from that of its ancestor, allowing the scoring scheme to
``ignore'' the information which it contains.  In the trivial case
where the outgroup branch length is zero (so the ancestor is identical
to the outgroup), we can perfectly reconstruct the ancestral structure
(as expected), but we cannot perfectly reconstruct the three-way
multiple alignment (since there are still two finite-length branches,
which on average contribute a greater-than-zero rate of
unreconstructable mutations).

These results correspond to a best-case scenario in which we have a
perfectly-parametrized evolutionary model and exactly-known descendant
structures.  Recent results demonstrating that inference of sequence
alignment is robust to mis-parametrized probabilistic models
\cite{Lunter2007} suggest that the same may be true for probabilistic
inference of ancestral structures.
%The effect of imperfect knowledge of the ancestral structures has not been explored.
% [commented out because the following tests DO address this in a roundabout way; what we mean is that indiegram has not been tested when leaf structures are unknown]

To compare \indiegram\ to simpler reconstruction methods, we also measured the alignment accuracy using various simpler statistical reconstruction techniques.
We evaluated three maximum likelihood inference algorithms that use simpler gap-aware phylogenetic models
and one posterior decoding algorithm that uses a non-phylogenetic sum-of-pairs scoring scheme.
These results are plotted on \figref{simulations} alongside the \indiegram\ accuracies.
In detail, the reconstruction methods used were

\begin{description}
\item{TKF91 ML:} with the TKF91 model \cite{ThorneEtal91}, the \handel\ package \cite{HolmesBruno2001,Holmes2003,Holmes2007} was used to align the three extant sequences and reconstruct the ancestor.
...
\item{Long Indel ML:} with a single-event trajectory approximation to the long indel model \cite{MiklosLunterHolmes2004}, the \handel\ package was used to align the three extant sequences and reconstruct the ancestor.
...
\item{Stemloc ML:} the \stemloc\ program was used to align the three sequences via single-linkage clustering with a Pair SCFG \cite{Holmes2005}.
The structures of the leaf sequences were provided, but not the phylogenetic branch lengths.
{\bf This model improves on the previous model (Long Indel) by introducing RNA secondary structure, though it no longer models phylogeny as rigorously as the Long Indel model.}
\item{Stemloc AMA:} the \stemloc\ program was used to align the three sequences in ``sequence annealing'' mode,
...
The structures of the leaf sequences were provided, but not the phylogenetic branch lengths.
{\bf This program uses the previous model but abandons phylogeny altogether, instead attempting to maximize a non-phylogenetic alignment accuracy metric.}
\end{description}

We did not measure the accuracy of the reconstructed ancestral structure in these tests (as we did in the \indiegram\ test),
since \handel\ does not consider structures, whereas \stemloc\ and \stemlocama\ do not provide ancestral reconstructions.


{\bf SUMMARY OF THESE RESULTS}
% TKF is less accurate than indiegram, although for short branch lengths it is almost identical; the TKF and indiegram accuracies diverge at longer branch lengths
% Long indel?
% Stemloc?
% StemlocAMA?
...
\begin{figure}[!ht]
  ...
  \caption{
    ...
    maximum-likelihood reconstruction thereof, estimated by aligning the two structures using \stemloc\ \cite{Holmes2005}.
    ``Alignment accuracy'' is the Alignment Metric Accuracy (AMA),
    effectively the probability that any given residue is correctly
    aligned, averaged over all pairs of sequences
    (see \cite{SchwartzMyersPachter2006} for a precise definition).
}
