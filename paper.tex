...
The ensemble grammar contains many rules that can be applied redundantly,
together or in isolation, to generate subtrees of the parse tree that do
not generate any terminals, and could be replaced with simpler rules.
In fact, this is a general property of SCFGs with null states and/or bifurcations.
In this subsection, we describe how to transform the SCFG so as to eliminate such redundant rules.
We begin by identifying two types of redundant parse-structures: {\bf empty bifurcations} and {\bf empty paths}.
