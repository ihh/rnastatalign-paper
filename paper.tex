...
The ensemble model contains empty bifurcations {\em and} empty paths.
...
{\bf empty paths} which do not emit any terminal symbols.
...
We refer to empty paths and empty bifurcations as {\bf null cycles}.
They occur (in simpler form) in Hidden Markov Models (which are special cases of SCFGs)
and independent-sites models (which can be viewed as special cases of HMMs).
In all such models, null cycles represent histories that are valid according to the model, but cannot be resolved by direct observation.
...
Often, one can account for the likelihood of such events by exact marginalization.
For example, eliminating missing ``empty'' columns from an independent-sites model of sequence involves a straightforward application of a correction factor
...
summing over all null paths, yielding a geometric series that is solvable by matrix inversion
\cite{Holmes2003,Holmes2007,Lunter2007}; the ``wing retraction'' algorithms used in HMMER can be viewed as a special case of matrix inversion by iterated elimination \cite{hmmer}.

Algorithms for exact elimination of HMM null cycles by matrix inversion are implemented
...
Here, we show how null cycles can be removed systematically from SCFGs by two steps:
\begin{description}
...
% "Another heuristic that could be tried..."
