...
of alignment inference and reconstruction accuracy on the evolutionary
...
% SCFGs for RNA
Inspection of RNA alignments, for example in the \RFAM\ database
...
related RNAs.  The focus of the present work is to solve the second
...
(1) an algorithm that transforms a phylogenetic ensemble of pair grammars
...
we assess the conduct a simulation study to investigate the accuracy
with which we can hope to reconstruct ancestral structures over long
evolutionary time in the presence of structural divergence.
...
As discussed below, the complexity of our inference algorithms is
...
compatible with a curated multiple alignment.  Such constraints are
commonly used by programs for RNA sequence analysis \cite{RivasEddy2001,Holmes2005}.

The following sections introduce more precise definitions for two-sequence
models of RNA structure and proceed to outline our algorithms for
...
indels first in primary sequences, then in sequences with conserved secondary structure.
...
the joint distribution $P (X, Y | T)$ is modeled by a jointly-normalized Pair SCFG \cite{Durbin98}.
...
The stochastic state machine which generates parse-trees from the marginal distribution $P(X)$ is a {\bf singlet transducer}.
The machine that absorbs parse-trees $X$ and generates modified parse-trees $Y$ from the conditional distribution $P(Y | X, T)$ is a {\bf branch transducer}.
Singlet transducers only emit symbols to their output sequence;
branch transducers, in contrast, can both emit symbols to their output
and absorb symbols from their input sequence.

Much as the states of Pair HMMs with no null states, other than the $\Sstart$
and $\Send$ states, can be classified as $\Sinsert$ or $\Smatch$,
...
or emits no output symbol at all (the latter corresponds to a deletion).
...
In a conditionally-normalized Pair SCFG, the groups are finer-grained:
each co-normalized group includes all rules that can be applied to a given nonterminal, {\em for a given set of absorbed symbols}.
...
Given a phylogenetic tree and two-sequence model, we wish to obtain a
...
Throughout this paper we make frequently refer to two and
...
states, each branch transducer has $b$ states, and there are $N$
branches in the phylogeny, then an upper bound on the number of
...
\item {\bf Terminal Emission}:
  ...
  (albeit possibly being replaced by other terminal symbols as they're propagated down)
...
specifying an ordering on the nodes (preorder with respect to the
phylogenetic tree) and by having branch transducers ``pause'' in $\Swait$
...
The four types of allowed transitions listed above can be formalized simply.
...
including the pairwise model we use here.
...
algorithm given below for enumerating all allowed transitions of the
...
We can then use the Cocke-Younger-Kasami (CYK) algorithm to impute the
...
these unobserved fragments, the existence of which complicates the
inference algorithms.

Due to this possibility of unobserved ancestral sequence,
the standard CYK and Inside algorithms fail on the ensemble model.
Residues in the ancestor can be absent in all extant sequences;
the potential number of such unobserved ancestral residues is unbounded.
This manifests as cycles of $\Snull$ states within the multiple-sequence SCFG,
...
While there exists a straightforward fix for maximum-likelihood
inference with the CYK algorithm, the Inside algorithm requires
further computational machinery.

In order to correctly perform maximum-likelihood inference, we only
require that the lowest-probability parse corresponds to the parse
...
However, posterior-decoding inference, which uses the Inside and
Outside algorithms, required a modified DP algorithm which preserves
the entire probability distribution over parses; the transformations
just described to make CYK work properly are insufficient for the
...
algorithms due to the need to iterate over parts of the (large) DP
matrix.  The second approach, of modifying the structure of the SCFGs
themselves, has a closed-form solution for the case of HMMs, but to our
knowledge, efficient exact null-cycle elimination for SCFGs is an open
mathematical problem (complicated by the presence of null cycles
involving bifurcations).

We therefore developed an iterative approximation to remove null cycles from
...
Null cycles in SCFGs constitute a ``missing data'' problem, wherein
the unobserved values of variables contribute to probabilities of
observed events. Such missing data problems are common in
probabilistic models of sequences and can
frequently be dealt with by exactly marginalizing over the
probabilities of the unobserved variables.  For example, eliminating
missing ``empty'' columns from an independent-sites model of sequence,
wherein alignment columns are independent, is straightforward
\cite{RivasEddy2008} and missing emissions in a HMM can be dealt with by matrix inversion
\cite{Holmes2003,Holmes2007,Lunter2007} or the ``wing retraction''
techniques used in HMMER \cite{hmmer}.
Algorithms for exact elimination of HMM null cycles by
matrix inversion are well-known \cite{Holmes2003} and are implemented
by programs such as PhyloComposer \cite{Holmes2007} and
...
However, these solutions do not easily generalize to SCFGs, due to the
possible presence of null bifurcations.

Null cycles can be removed from SCFGs by (i) separating bifurcations
...
two nonempty children and then (ii) encapsulating all paths that do
not emit any terminal symbols (and may therefore cause cyclic
dependencies in the DP matrix) as effective direct transitions, with
probabilities obtained by geometric series summation of the grammar's
transition matrix.

Both (i) empty bifurcations and (ii) paths without terminal symbols
generically appear in the ensemble model.
An empty bifurcation occurs when a child branch of a bifurcation state
...
bifurcation $N_1 \rightarrow B \rightarrow (N_2 \,\, N_3) \rightarrow
...
Similarly, our model construction procedure generically produces many
``windback'' null transitions to non-emitting placeholder
states. If $\Semit$ states $E_1$ and $E_2$ are connected in the state
graph by $\Snull$ states $N_1$ and $N_2$, then the transition $E_1
\rightarrow N_1 \rightarrow N_2 \rightarrow E_2$ with  probability
$P(E_1 \rightarrow N_1) \cdot P(N_1 \rightarrow N_2) \cdot P(N_2
\rightarrow E_2)$ can be replaced with a transition $E_1 \rightarrow
...
Algorithms \ref{alg:inside}--\ref{alg:cyktraceback} give the Inside, CYK,
Outside and CYK traceback algorithms for a three-taxon star phylogeny,
constrained using the ``fold envelopes'' described below.
...
\end{align*}
where the quantities $\Delta^{(X)}_L(\bvec{b})$ and $\Delta^{(X)}_R(\bvec{b})$ are the lengths of the left and right emissions of the ensemble state $b$ to the sequence $X$.
...
\end{align}
We generally write out explicit subsequence coordinate pairs $(i,j)$ when their usage will make mathematical formulae clearer and fold-envelope labels $n^{(X)}$ when writing pseudocode.
...
The CYK probabilities for indices $\gamma_{\bvec{a}} \left(n^{(X)}, n^{(Y)}, n^{(Z)}\right)$ then represent 
...
distributions with parameters $\kappa_1$ and $\kappa_2$.  Single bases
are inserted and deleted in loops with rates $\lambda_1$ and $\mu_1$;
similarly, base-pairs are inserted and deleted in stems with rates
$\lambda_2$ and $\mu_2$.  Insertions of a new stem into an existing
...
and the insertion and deletion rates $\lambda_1$ and $\mu_1$ of the model.
The length of ancestral sequences is geometric in $\kappa_1$ (\tabref{tkfstsinglet}), defined as
\[
...
\begin{align}
  \alpha_1 (t) &= \exp \left( -\mu_1 t \right) \nonumber \\
  \beta_1 (t) &= \frac{\lambda_1 \left( 1 - \exp \left((\lambda_1 - \mu_1) t \right) \right)}{\mu_1 - \lambda_1 \exp \left( (\lambda_1 - \mu_1) t \right) } \nonumber \\
  \gamma_1 (t) &= 1 - \frac{\mu_1 \left( 1 - \exp \left((\lambda_1 - \mu_1) t \right) \right)}{\left( 1 - \exp (- \mu_1 t) \right) \left(\mu_1 - \lambda_1 \exp \left( (\lambda_1 - \mu_1) t \right) \right) } \nonumber
\end{align}
...
For example, it fails to model certain phenomena observed in natural
...
using an MCMC kernel similar to that of \cite{JensenHein2002}.

The statistics of the TKFST model on the three-taxon phylogeny
illustrate the advantages of our procedure for model
construction.  While the singlet and branch transducers are relatively simple---the singlet transducer,
shown in \tabref{tkfstsinglet}, has 7 total states and 2 bifurcation states and
the branch transducer, shown in \tabref{tkfstbranch}, has 21 total states
and 6 bifurcation states---the ensemble model of three extant sequences 
is very complex.
The naive exponential upper-bound gives a maximal state space of size $O(7 \cdot 21^{3}) \Rightarrow 6 \cdot 10^4$ states.
...
estimated with the exponential calculation).  After performing the
transformations described in ``Eliminating null cycles in SCFGs'' to
eliminate useless windback states,
the ensemble model has a reduced state space with 230 states,
albeit at the cost of extra transitions, bring the total to 1,789 transitions
(here we are trading reduced memory complexity, which is linear in the number
of states, for increased time complexity, which is linear in the number of transitions).
Note that both before and after the reduction in complexity,
the total number of states and transitions are less than the approximate bounds
of $O(a \cdot b \cdot N) = O(7 \cdot 21 \cdot 3) \Rightarrow 441$ states
and
$O(a \cdot b^2 \cdot N) = O(7 \cdot 21^2 \cdot 3) \Rightarrow 9,261$ transitions
given in ``The composition algorithm.''

% to do: dump output of tripletscfg.pl --tkfst into supplement
Text S? contains the complete model specification, including states and transitions,
produced by our model composition algorithm.  The extreme complexity of the
...
and ancestral structure of three extant sequences (Algorithms \ref{alg:cyk}-\ref{alg:cyktraceback})
in a C++ program (\indiegram).
...
structures of individual RNAs are rare, curated deep sequence
alignments, such as those constructed for ribosomal RNAs
\cite{Gutell93}, are more frequently available.  By constraining the
...
We used the TKF Structure Tree model on a 3-taxon phylogeny to 
reconstruct most-probable ancestral structures of three \emph{nanos} 3'
translational control elements (TCEs) and three tRNAs, two small RNAs whose
structures are well-known and which are accessible given
current computational limitations. While the phylogenetic trees
relating the tested sequences have short branch lengths, making the
reconstruction problem easy by forcing the reconstructed structures to
be essentially-identical to those of one of the extant RNAs
(reconstructed structures not shown; see \indiegramurl), these
are nonetheless useful proof-of-concept datasets.

\tabref{complexities} shows estimates of the memory and time required
to reconstruct biologically-interesting subunits of the two RNAs
tested, the \emph{nanos} 3' translational control element and tRNAs,
...
3-taxon phylogenies, where we kept the branch lengths of the two
sibling species constant and varied the branch length of the
outgroup.  We fixed the tree topology as $((X:1,Y:1):1,Z:z)$, where
the branch length $z$ of the outgroup was varied between 0.5 and 3.0
and a branch length of 1 corresponds to one expected substitution per site
in loop sequence.  We chose the parameters of the TKFST model to
...
stems, sequences of length $\in [30, 70]$, loops of length $\in [3, 10]$ and
stems of lengths $\in [1, 20]$.

We expected to find a ``transition point'' in our reconstruction
accuracy near the point $z \simeq 1$, corresponding to an outgroup
distance equal to the distance from either of the sibling species to
the most recent common ancestor.  Our results, shown in
\figref{simulations}, indicate such a transition point for alignment
accuracy but not for ancestral reconstruction accuracy.
% to do: here now

\cite{Mossel2003}

While these results correspond to a best-case scenario in which we
have a perfectly-parametrized evolutionary model, recent results
demonstrating that inference of sequence alignment is surprisingly
robust to mis-parametrized models \cite{Lunter2007} suggest that the same
may be true for inference of ancestral structures.
...
A conditional framework similar to ours in some respects is described by Sakakibara {\em et al} \cite{Sakakibara2003}.
The statistical inference algorithms for multiple-sequence SCFGs are closely related to the ``protosequence algorithm'' of Sankoff \cite{Sankoff85},
...
% to do: discuss the possibility of progressive reconstruction a la Ortheus
...
would require resources far in excess of what are currently available;
...
\begin{figure}[!ht]
  ...
  \caption{
    \textbf{Dependence of reconstruction accuracy on outgroup branch
      length.}
    We allowed the outgroup branch length $z$ to vary within $[0.5,
    3.0]$, where a branch length of 1 corresponds to one expected
    substitution per site in loop sequence.  We divided this interval
...
\begin{table}[!ht]
  ...
  \begin{flushleft}
    ...
    The singlet transducer generates ancestral RNA sequences and
    structures.
    We use the notation of formal grammars to represent state
    transformation rules; for example, the rule $I_L \rightarrow
    u\,\,I_L$ corresponds to (in a Pair HMM) an $\Sinsert$ state $I_L$
    emitting a nucleotide $u$ and then making a self-transition.
    Both loop ($L$ and $I_L$) and stem ($S$ and $I_S$) sequence evolve as TKF sequences
    with length parameters $\kappa_l$ and $\kappa_s$ (defined in ``A simple model of RNA structural evolution'').
    $\pi_l(u)$ and $\pi_s(uv)$ are the equilibrium distributions of unpaired nucleotides $x$ and
...
\begin{table}[!ht]
  ...
  \begin{tabular}{|rcl|r||rcl|r|} \hline
    ...
    $W_L$ & $\rightarrow$ & $u \,\, w \,\, M_L$ & $\alpha_l(t) \cdot M_l(u,w)$ & $W_S$ & $\rightarrow$ & $u \,\, w \,\, M_S \,\, x \,\, v$ & $\alpha_s(t) \cdot M_s(uv,wx)$ \\ \hline
    & $|$ & $u \,\, D_L$ & $1-\alpha_l(t)$ & & $|$ & $u \,\, D_S \,\, v$ & $1-\alpha_s(t)$ \\ \hline
  ...
  \begin{flushleft}
    ...
    $M_l(u,w)$ and $M_l(uv,wx)$ are the conditional distributions
    (i.e. match probabilities) of, respectively, descendant unpaired
    nucleotide $w$ given an ancestral unpaired nucleotide $u$ and
    descendant paired nucleotides $(w,x)$ given ancestral nucleotides
    $(u,v)$. The functions $\alpha_n(t)$, $\beta_n(t)$ and
    $\gamma_n(t)$ are parametrized by the insertion and deletion rates
